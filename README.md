...
```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/jufeic/dotfiles/refs/heads/macos/install.sh)"
```
3. Or run this shell command to install the dotfiles for all platforms:
```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/jufeic/dotfiles/refs/heads/macos/install.sh)" _ all
```
