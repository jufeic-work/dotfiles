...
-- like Jip
-- Normal mode mapping for J
-- This means that you need to use JJ now if you want the J behavior prior to the mapping
vim.keymap.set("n", "J", [[:set operatorfunc=JoinOperator<CR>g@]], { silent = true, expr = false })

-- Operator-pending mode mapping to make J act like the motion 'j'
vim.keymap.set("o", "J", "j", { noremap = true })

-- Define the function in Vimscript because operatorfunc needs it that way
vim.cmd([[
...
-- please do nothing when pressing capital q
...
-- improvement for vertical movement to always have a centered cursor when moving
vim.keymap.set("n", "<C-d>", "<C-d>zz")
vim.keymap.set("n", "<C-u>", "<C-u>zz")
-- this remap if OP if you yank text and want to replace another part with it
-- the default behavior for yiw viw p is that you paste your yanked text and the
-- removed text goes in this register
-- with this new behavior if you press <leader>p instead the deleted text goes in
-- the trash register so the originally yanked text is still in the register
-- i have modified this to behave with the yanked text stay in the register as default
-- and only get the other behavior with leader before
...
-- Diagnostic keymaps
-- This setting is needed to enable the inline diagnostics
...
require("lazy").setup({
	...
	-- in your Lua plugin spec (e.g. in lazy.lua)
	{
		"ThePrimeagen/harpoon",
		branch = "harpoon2", -- use 'harpoon2' for latest rewrite, otherwise omit
	...
	},
	{
		...
		opts = {
			search = {
				-- Match only letters at the beginning of a word
				mode = function(str)
					return "\\<" .. str
				end,
			},
			modes = {
				-- Disable f,F,t,T flash behavior
				char = {
					enabled = false,
				},
			},
  ...
  keys = {
	...
	{ -- Fuzzy Finder (files, grep, LSP, etc)
		...
		opts = {
			...
			-- files = { hidden = true },
			grep = {
				-- Also grep in hidden files / files in hidden folders (rg -n --hidden "" .)
				hidden = true,
			},
		},
		config = function(_, opts)
			...
			-- Keymaps (roughly mirroring your Telescope ones)
			...
			vim.keymap.set("n", "<leader>fv", fzf.grep_visual, { desc = "[F]ind [V]isual" })
			...
			-- Fuzzy search in current buffer (Telescope's current_buffer_fuzzy_find)
			-- vim.keymap.set("n", "<leader>/", function()
			-- 	fzf.blines({ query = vim.fn.expand("<cword>") })
			-- end, { desc = "[/] Fuzzily search in current buffer" })
			vim.keymap.set("n", "<leader>/", fzf.blines, { desc = "S" })

			-- Live grep in open files (Telescope grep_open_files)
			-- vim.keymap.set("n", "<leader>f/", function()
			-- fzf.live_grep({ grep_open_files = true, prompt = "Live Grep in Open Files" })
			-- 	fzf.live_grep({ prompt = "Live Grep in Open Files" })
			-- end, { desc = "[S]earch [/] in Open Files" })

			-- Search your Neovim config files
			vim.keymap.set("n", "<leader>sn", function()
		...
		config = function()
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(event)
					...
					map("gT", fzf.lsp_typedefs, "Type [D]efinition")

					-- Fuzzy find all the symbols in your current document.
					--  Symbols are things like variables, functions, types, etc.
					map("<leader>fs", fzf.lsp_document_symbols, "[D]ocument [S]ymbols")
